...
class HashtagTrendReweet3(TrendGraph):
    ...
    _node_color = ("type", dict(status="blue", source="red", target="lime", TimeIndicator="purple"))
    _edges_color = ("type", dict(source="gold", target="turquoise"))

    def __init__(self, analytics_meta):
        super(HashtagTrendReweet3, self).__init__(analytics_meta)

    @classmethod
    def get_type(cls):
        return "Hashtag User Trend Retweet 3 Graph"

    @classmethod
    def get_args(cls):
        return cls.__arguments + super(HashtagTrendReweet3, cls).get_args()
    ...
    def build_graph(self, cursor, time_interval, classification_system):
        ...
        for status_json in cursor:
            ...
            self.link_node_to_history(G, history, node_id, source_user_id)
            G.add_edge(node_id, status_id, type="source")
            ...
            if retweet is not None:
                ...
                if user_id != source_id:
                    ...
                    G.add_edge(status_id, target_node_id, type="target")
        return G, start_date
